...
-- Constraints for some column (zip, phone)
  -- zip length must be 5
  
-- Steps to fix the data
  -- 1. Create a backup table.
  --  Create table <table name>
  -- 2. If it possible create a new column and work in the column
  --  Fixing the st values.
  
  -- Review the update
    
  
-- Challenge
-- Add new column meat_processing (boolean)
  -- set this column to true if the company has "Meat Processing"
  
  ALTER TABLE meat_poultry_egg_establishments ADD meat_processing boolean;
  SELECT meat_processing,* from meat_poultry_egg_establishments limit 10;
  
  UPDATE meat_poultry_egg_establishments
  SET meat_processing = true
  where activities like '%Meat Processing%';
  
  SELECT * FROM meat_poultry_egg_establishments WHERE meat_processing = true;
  
-- For delete duplicates
  -- Use an aggregate fucntion to find out the values that you want to keep (max, min)
  
-- Transactions
  -- Help us to group db operations
  -- Rollback
  -- Use transactions all time you are updating data.
  -- Commit
